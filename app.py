...
# crear un contenedor de un solo elemento
...
#while True: 
    ...
    df['rutas_atrasadas'] = df['age'] * np.random.choice(range(1,12)) #sacamos la media
    df['nuevo_balance'] = df['balance'] * np.random.choice(range(1,5)) #sacamos la media

    # creando el KPI

    avg_edad = np.mean(df['rutas_atrasadas']) 
    ...
    with placeholder.container():
        ...
        with fig_col1:
            ...
            fig = px.density_heatmap(data_frame=df, y = 'rutas_atrasadas', x = 'estado_civil')
            st.write(fig)
        with fig_col2:
